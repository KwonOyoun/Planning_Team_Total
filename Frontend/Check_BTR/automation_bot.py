...
# [중요] PyInstaller 실행 시(frozen 상태), 브라우저를 임시 폴더가 아닌 시스템 글로벌 경로에서 찾도록 강제함
...
import re
import json
from . import draft_handler  # [추가] 분리된 핸들러 임포트
...
class GroupwareBot:
    ...
    def start(self):
        ...
        # [수정] 브라우저 실행 시도 및 미설치 시 자동 설치 로직 추가
        try:
            self.browser = self.playwright.chromium.launch(
                headless=self.headless, 
                args=["--window-position=0,0", "--window-size=1800,1200"]
            )
        except Exception as e:
            print(f"⚠️ Browser launch failed: {e}")
            if "Executable doesn't exist" in str(e) or "not found" in str(e):
                print("🌐 Installing Chromium browser... (This may take a while on first run)")
                try:
                    import sys
                    ...
                    try:
                        ...
                        if se.code != 0:
                            print(f"⚠️ Install exited with code {se.code}")
                    finally:
                        sys.argv = old_argv
                    ...
                    self.browser = self.playwright.chromium.launch(
                     ...
                     print(f"❌ Failed to install browser: {install_err}")
                     raise install_err
            else:
                raise e
