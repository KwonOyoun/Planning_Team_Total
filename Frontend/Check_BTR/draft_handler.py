...
from . import content_generator  # [추가] 새로 만든 파일 임포트
...
def _extract_purpose_from_title(title: str) -> str:
    """
    제목에서 출장목적 추출
    Format: [Category] YYMMDD Purpose 국내출장...
    Logic:
      1. Remove [ ... ]
      2. Remove YYMMDD (6 digits)
      3. Remove '국내출장' and everything after
      4. Strip
    """
    # 1. Remove [ ... ]
    t = re.sub(r"\[[^\]]+\]", "", title)
...
def execute_drafting(page, app_id, trip_data=None):
    ...
    try:
        ...
        # [추가] 제목에서 출장목적 추출
        trip_purpose = _extract_purpose_from_title(new_title)
        print(f"[Draft Handler] Extracted Purpose: {trip_purpose}")
        
        if trip_data:
            trip_data['trip_purpose'] = trip_purpose
        ...
        # [수정됨] 4번 단계(참조문서)가 끝난 후, 본문 주입 실행
        print("[Draft Handler] Injecting Report Body Content...")
    
        # ---------------------------------------------------------------
        # 5. 본문 내용 채우기 (Dynamic HTML Injection with Cleanup)
        # ---------------------------------------------------------------
        print("[Draft Handler] Generating & Injecting Full Report HTML (Dynamic V2)...")
        
        # 1) HTML 생성
        full_html = content_generator.generate_html(trip_data)
        
        # 2) 에디터에 주입 (기존 표 삭제 후 추가)
        _inject_html_with_cleanup(page, full_html)
...
# -------------------------------------------------------------------------
# [누락된 함수 추가] 메인 파일 맨 아래에 붙여넣어 주세요.
# -------------------------------------------------------------------------
def _inject_html_to_editor(page, html_content):
    """
    생성된 HTML 문자열(html_content)을 에디터(iframe)의 body 끝에 삽입합니다.
    """
    print("[Draft Handler] Injecting generated HTML into editor...")
    
    # 에디터 로딩 대기 (안전장치)
    try:
        page.wait_for_selector("#editorView_1", state="attached", timeout=5000)
    except:
        print("[Draft Handler] Warning: Editor iframe not found via selector.")

    # JS로 insertAdjacentHTML 실행 (기존 헤더 유지하며 내용 추가)
    js_script = """
        (html) => {
    ...
    try:
        result = page.evaluate(js_script, html_content)
        if result:
            print("[Draft Handler] HTML Injection Success.")
            return True
        else:
            print("[Draft Handler] HTML Injection Failed (DOM Error).")
            return False
    except Exception as e:
        print(f"[Draft Handler] HTML Injection Error: {e}")
        return False

def _inject_html_with_cleanup(page, html_content):
    """
    [New V2] 재귀적으로 모든 프레임을 탐색하여 '출장기간' 등이 포함된 실제 에디터 문서(Body)를 찾습니다.
    찾아낸 문서에서 기존 표를 삭제하고 새 HTML을 주입합니다.
    """
    print("[Draft Handler] Injecting HTML with Recursive Cleanup strategy...")

    js_script = """
    (html) => {
        // 1. Recursive Search for the Document containing the specific table
        function findTargetDoc(win) {
            try {
                var doc = win.document;
                // Check local content
                var allTDs = doc.getElementsByTagName('td');
                for (var i = 0; i < allTDs.length; i++) {
                    // Check for key marker of the default template
                    if (allTDs[i].innerText.replace(/\\s/g, '').includes('출장기간')) {
                        return doc; // Found the editor document!
                    }
                }
                ...
                for (var i = 0; i < frames.length; i++) {
                    try {
                        var childRes = findTargetDoc(frames[i].contentWindow);
        ...
        // 2. Execute Search
        var targetDoc = findTargetDoc(window);
        
        // Fallback: If not found, try the known ID 'editorView_1' explicitly
        if (!targetDoc) {
            var iframe = document.getElementById('editorView_1');
            if (iframe) {
        ...
        if (targetDoc && targetDoc.body) {
            // 3. Perform Cleanup & Injection on the Found Document
            var tables = targetDoc.getElementsByTagName('table');
            var toRemove = [];
            
            // Collect tables to remove
            for (var i = 0; i < tables.length; i++) {
                var txt = tables[i].innerText.replace(/\\s/g, '');
                if (txt.includes('출장기간') && txt.includes('출장자')) {
                    toRemove.push(tables[i]);
                }
                else if (txt.includes('교통비') && txt.includes('식비')) {
                    toRemove.push(tables[i]);
                }
                else if (txt.includes('산출내역')) {
                     toRemove.push(tables[i]);
                }
            }
            ...
            for (var i = 0; i < toRemove.length; i++) {
                // Try to remove preceding title DIVs
                var prev = toRemove[i].previousElementSibling;
                if (prev && (prev.innerText.replace(/\\s/g, '').includes('출장개요') || prev.innerText.replace(/\\s/g, '').includes('산출내역'))) {
                    prev.remove();
                }
                toRemove[i].remove();
            }
            ...
            return { success: true, removed: removedCount, docFound: true };
        }
        
        return { success: False, message: "Target document not found" };
    }
    """
    try:
        # Pass html_content
        ...
        if result and result.get('success'):
            print(f"[Draft Handler] Injection Success. Target Doc Found. Removed {result.get('removed')} existing tables.")
            return True
        else:
            print(f"[Draft Handler] Injection Failed. Result: {result}")
            return False
    except Exception as e:
        print(f"[Draft Handler] Injection Error: {e}")
        return False

# Legacy function kept for reference but not used
def _fill_report_body_js(page, trip_data):
    pass


# 기존 _inject_report_body 함수는 삭제하거나 주석 처리하세요.
