...
from flask import (
    Blueprint, Response, jsonify, redirect, render_template, request, url_for, send_from_directory
)
...
def _parse_any_dt(s: str) -> datetime | None:
    ...
    if "." in guess:
        ...
        if m: guess = m.group(1)
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M"):
        try: return datetime.strptime(guess, fmt)
...
def _load_json_safe(path: Path, fallback: dict) -> dict:
    ...
    if path.exists() and path.stat().st_size > 0:
        for _ in range(3):
            ...
            except json.JSONDecodeError: time.sleep(0.15)
            except Exception: break
    bak = path.with_suffix(path.suffix + ".bak")
    if bak.exists() and bak.stat().st_size > 0:
        try: return _read(bak)
        except Exception: pass
...
def _pick_date_str(item: dict) -> str | None:
    if item.get("date"): return str(item["date"])
    meta = item.get("meta") or {}
    for k in ("공고일자", "등록일", "게시일"):
        if meta.get(k): return str(meta[k])
    if meta.get("notice_posted_at"): return str(meta["notice_posted_at"])
...
def index():
    ...
    return render_template("monitor_index.html", groups=groups, stats=stats, params=params)

@Monitor_GOV_bp.route("/api/notices")
def api_notices():
    return jsonify(load_results())

@Monitor_GOV_bp.route("/edu")
def edu():
    data = load_events()
    q = request.args.get("q", "").strip().lower()
    ...
    return render_template("monitor_events.html", items=items, stats=stats, params={"q": q})

@Monitor_GOV_bp.route("/api/events")
def api_events():
...
def run_collection(threshold: int = 0, max_pages: int = 10):
    import sys
    # Add Monitor_GOV root to path to import collect
    sys.path.append(str(BASE.parent))
    try:
        from main import collect
    except ImportError:
        print("[ERROR] Could not import collect from main.py in Monitor_GOV")
        return
    ...
    if tmp_out.exists():
        try: payload = json.loads(tmp_out.read_text(encoding="utf-8"))
        except Exception: pass
    if not payload:
...
def run_events_collection(max_pages: int = 2):
    ...
    try:
        ...
        print("[ERROR] Could not import collect_edu from main.py in Monitor_GOV")
        return

    tmp_out = EVENTS_JSON.with_suffix(EVENTS_JSON.suffix + ".tmp_collect")
    payload = collect_edu(max_pages=max_pages, out_path=tmp_out)
    if tmp_out.exists():
        try: payload = json.loads(tmp_out.read_text(encoding="utf-8"))
        except Exception: pass
    if not payload:
...
@Monitor_GOV_bp.route("/refresh", methods=["POST"])
def refresh_all():
    if not _FLAGS["all"]:
        _FLAGS["all"] = True
        def _job():
    ...
    return redirect(url_for("Monitor_GOV.index"))

@Monitor_GOV_bp.route("/refresh/notices", methods=["POST"])
def refresh_notices():
...
@Monitor_GOV_bp.route("/refresh/events", methods=["POST"])
def refresh_events():
    if not _FLAGS["events"]:
        ...
        def _job():
            try:
                ...
                _FLAGS["events"] = False
        threading.Thread(target=_job, daemon=True).start()
    return redirect(url_for("Monitor_GOV.edu"))
...
@Monitor_GOV_bp.route("/proxy/kiat/<contents_id>")
...
def _auto_refresh_loop():
    while True:
        try:
            ...
            print("[Monitor_GOV AUTO-REFRESH ERROR]", e)
        time.sleep(15 * 60)
